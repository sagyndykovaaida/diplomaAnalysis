...
def plot_intraday_data(df):  # Добавлен параметр title
...
def plot_weekly_data(df):  # Добавлен параметр title
    plt.figure(figsize=(14, 7))
    plt.plot(df.index, df['high'], label='High Price', color='green')  # Уточнение столбца
    # plt.title(title)  # Установка заголовка
    plt.xlabel('Week')
    plt.ylabel('Price in USD')
    plt.legend()
...
def plot_daily_data(df):  # Добавлен параметр title
    plt.figure(figsize=(14, 7))
